          ...
          <tbody className="divide-y divide-slate-900">
            {events.map((event) => { const isOpen = expanded === event.eventId; const destination = destinationLabel(event); return <Fragment key={event.eventId}>
              <tr className="group hover:bg-slate-900/55">
                ...
                <td className="max-w-[340px] px-3 py-2.5 font-mono text-xs font-medium text-slate-100" title={destination}><span className="block truncate">{destination}</span>{event.destinationIp && !event.destinationHost && !event.detectedDomain && <IpOwner ip={event.destinationIp} />}</td>
